...
static struct usb_device_id ch37x_table[] = {
	{ USB_DEVICE(0x1a86, 0x55e0) },
	{ USB_DEVICE(0x4348, 0x55e0) },
	{} /* Terminating entry */
};
...
static ssize_t ch37x_bulk_write(struct usb_ch37x *dev, int num, void *buf, unsigned long count)
{
	...
	rv = usb_bulk_msg(dev->udev, usb_sndbulkpipe(dev->udev, dev->bulk_out_endpointAddr[num]), buffer,
			  bytes_to_write, &bytes_write, dev->writetimeout);
	if (rv == 0) {
		rv = bytes_write;
	}
...
long ch37x_ioctl(struct file *ch37x_file, unsigned int cmd, unsigned long arg)
{
	...
	case GetDeviceID:
		...
		break;
	case GetDeviceDes:
		retval = usb_get_descriptor(dev->udev, USB_DT_DEVICE, 0x00, &dev->udev->descriptor,
					    sizeof(dev->udev->descriptor));
		if (retval)
			retval = copy_to_user((u8 __user *)arg, &dev->udev->descriptor, sizeof(dev->udev->descriptor));
		break;
...
#ifdef CONFIG_COMPAT
long ch37x_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	return ch37x_ioctl(filp, cmd, arg);
}
#endif
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = ch37x_compat_ioctl,
#endif
